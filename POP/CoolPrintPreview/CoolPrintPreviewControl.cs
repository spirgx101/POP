...
namespace CoolPrintPreview
{
    ...
    internal class CoolPrintPreviewControl : UserControl
    {
        ...
        //private Object thisLock3 = new Object();
        //private Object thisLock4 = new Object();
        ...
        public DataTable FormatHeaders { get; set; }
        public DataTable FormatDetails { get; set; }
        public string Paper { get; set; }
        public int Max_Page { get; set; }
        public bool StopPrint { get; set; }
        ...
        public void RefreshPreview()
        {
            ...
            if (_doc != null)
            {
                ...
                try
                {
                    _cancel = false;
                    _rendering = true;
                    _doc.DocumentName = Print_Source;
        ...
        public void Set_Print_Document()
        {
            DataTable dtPage = DataTableFilterSort(PrintData, "PAGE_ID = " + StartPage.ToString(), "PRINT_SEQ asc");
            PageLayout pageLayout = new PageLayout(dtPage, FormatHeaders, FormatDetails, Paper, Printer);
            this.Document = pageLayout.Get_Document();
            Refresh();
        }
        ...
        public void Print()
        {
            ...
            switch (ps.PrintRange)
            {
                case PrintRange.AllPages:
                    //Document.Print();
                    //Print_Document(1, Max_Page);
                    return;
                case PrintRange.CurrentPage:
                    //first = last = StartPage;
                    //Print_Document(StartPage, StartPage);
                    break;
                case PrintRange.Selection:
                    ...
                    if (ZoomMode == ZoomMode.TwoPages)
                    {
                        ...
                        //Print_Document(StartPage, Math.Min(StartPage + 1, Max_Page));
                    }
                    break;
                case PrintRange.SomePages:
                    //first = ps.FromPage - 1;
                    //last = ps.ToPage - 1;
                    //Print_Document(ps.FromPage, ps.ToPage);
                    break;
            }
        ...
        {
            Thread thPrint = new Thread(delegate () {
                int i = 0;
                for (i = setting.FromPage; i < setting.ToPage + 1; i++)
                {
                    if (StopPrint)
                    {
                        break;
                    ...
                    else
                    {
                        try
                        {                          
                            DataTable dtPage = DataTableFilterSort(PrintData, "PAGE_ID = " + i.ToString(), "PRINT_SEQ asc");
                            PageLayout pageLayout = new PageLayout(dtPage, FormatHeaders, FormatDetails ,Paper, setting.Printer);

                            using (PrintDocument pd = pageLayout.Get_Document())
                            {
                                pd.DocumentName = Print_Source;
                                pd.PrintController = new StandardPrintController();
                                pd.PrinterSettings.PrinterName = setting.Printer;
                                pd.DefaultPageSettings.PaperSource = setting.Source;
                                pd.Print();
                            }
                        ...
                        //StartPage = i;
                        //Document.PrinterSettings.PrinterName = setting.Printer;
                        //Document.DefaultPageSettings.PaperSource = setting.Source;
                        //var dp = new DocumentPrinter(this, i, i);
                        //dp.Print();
                    }
                }

                if (i == setting.ToPage + 1) MessageBox.Show("列印完成。");
            });
            thPrint.Start();         
        }
        #endregion
        ...
        */
        ...
        {           
            return page > 0 && page <= PageCount ? _img : null;    //每次只讀取一張圖片            
        }
        ...
        void UpdatePreview()
        {
            ...
            if (_startPage > PageCount) _startPage = PageCount;
